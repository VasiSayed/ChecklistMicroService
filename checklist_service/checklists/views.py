...
#            loc_q = Q()
#            if access.get('flat_id'):
#                loc_q &= Q(flat_id=access['flat_id'])  # can access only that flat
#
#            elif access.get('zone_id'):
...
#        room_ids = checklists.values_list('room_id', flat=True).distinct().exclude(room_id__isnull=True)
#        room_details = {}

#        for room_id in room_ids:
#            try:
#                room_resp = requests.get(f"{self.ROOM_SERVICE_URL}{room_id}", headers=headers, timeout=5)
